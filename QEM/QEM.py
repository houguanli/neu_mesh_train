...
from obj_loader import a_3d_model
    ...
    def update_plane_equation_parameters(self, need_updating_loc):
        ...
        for i in need_updating_loc:
            if self.status_faces[i] == -1:
                ...
                self.plane_equ_para[i, :] = calculate_plane_equation_for_one_face(point_1, point_2, point_3)
...
class QEM_driver:
    def __init__(self, input_filepath, threshold=0.3, simplify_ratio=0.1, output_filepath=None):
        if simplify_ratio > 1 or simplify_ratio <= 0:
            sys.exit('Error: simplification ratio should be (0<r<=1).')
        if threshold < 0:
            sys.exit('Error: threshold should be (>=0).')
        ...
        self.model = mesh_simplify(input_filepath, threshold, simplify_ratio)
        if output_filepath is None:
            print("not clarify output path, will overwrite old one")
            output_filepath = input_filepath
        self.output_filepath = output_filepath

    def run_QEM(self):
        print('simplifying with threshold {} simplify_ratio {} -----------------'.format(self.threshold, self.simplify_ratio))
        # Select all valid pairs.
        self.model.generate_valid_pairs()
        # Compute the optimal contraction target v_opt for each valid pair (v1, v2)
        # The error v_opt.T*(Q1+Q2)*v_opt of this target vertex becomes the cost of contracting that pair.
        # Place all the pairs in a heap keyed on cost with the minimum cost pair at the top
        self.model.calculate_optimal_contraction_pairs_and_cost()
        # Iteratively remove the pair (v1, v2) of least cost from the heap
        # contract this pair, and update the costs of all valid pairs involving (v1, v2).
        # until existing points = ratio * original points
        self.model.iteratively_remove_least_cost_valid_pairs()
        # Generate the simplified 3d self.model (points/vertices, faces)
        self.model.generate_new_3d_model()
        # Output the self.model to output_filepath
        self.model.output(self.output_filepath)
        print('done-------------------------')
